...
from models.models import CNN1DScorer


# Configuration variables for IDE usage (no CLI)
CSV_PATH = "data/keypoints.csv"  # path to your CSV file
EPOCHS = 20
BATCH_SIZE = 8
LEARNING_RATE = 1e-3
DEVICE = "cuda"  # or "cpu"
...
def _make_loaders_from_sequences(sequences, batch_size, shuffle=True):
    ...
    return torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_variable_seq)


def train_all_labels_cnn(csv_path: str, epochs: int, batch_size: int, lr: float, device: str):
    ...
    for label in labels:
        ...
        if len(ds) == 0:
            print(f"[CNN] Skipping label '{label}': no sequences.")
            continue
        ...
        if len(train_seqs) == 0:
            print(f"[CNN] Skipping label '{label}': not enough sequences for training.")
            continue

        # Infer input_dim and validate
        ...
        model = CNN1DScorer(input_dim=input_dim).to(device_t)
        ...
        for epoch in range(1, epochs + 1):
            ...
            msg = f"[CNN][Label {label}] Epoch {epoch}/{epochs} - train {train_loss:.4f}"
        ...
        ckpt_path = out_dir / f"cnn1d_scorer_label_{label}.pt"
        torch.save({"model_state": model.state_dict(), "input_dim": input_dim}, ckpt_path)
        print(f"Saved CNN1D model for label '{label}' to {ckpt_path}")


if __name__ == "__main__":
    # Train CNN for all labels found in the CSV with train/validation split
    train_all_labels_cnn(CSV_PATH, EPOCHS, BATCH_SIZE, LEARNING_RATE, DEVICE)
