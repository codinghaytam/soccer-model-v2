...
TEST_SPLIT = 0.2
SEED = 42
OUT_DIR = Path("models/cnn_lstm_motionbert")
...
NUM_JOINTS = 17
CHANNELS = 3   # x,y,conf
PERSONS = 1    # use single-person input shape for ActionNet
...
def _rotate_parts_in_seq(x: torch.Tensor) -> torch.Tensor:
    ...
    body_center = coords[:, BODY, :].mean(axis=1, keepdims=True)
    ...
    return torch.from_numpy(x_np).to(x.device, dtype=x.dtype)
...
# MotionBERT helpers (from train_multiclass_motionbert.py)
LIGHT_CFG_OVERRIDES = {"dim_feat": 128, "dim_rep": 256, "depth": 3, "num_heads": 4, "dropout_ratio": 0.5}
...
def _build_motionbert_backbone(cfg: dict):
    ...
    backbone = DSTformer(
        dim_in=dim_in, dim_out=dim_out, dim_feat=dim_feat, dim_rep=dim_rep, depth=depth,
        num_heads=num_heads, mlp_ratio=mlp_ratio, num_joints=num_joints, maxlen=maxlen,
        qkv_bias=qkv_bias, qk_scale=qk_scale, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        drop_path_rate=drop_path_rate, norm_layer=norm_layer, att_fuse=att_fuse,
    )
    return backbone
...
def _reshape_to_actionnet_input(x_bt: torch.Tensor, persons: int, joints: int, channels: int) -> torch.Tensor:
    ...
    return torch.cat([x1, x2], dim=1)  # (B,2,T,J,C)


class FusionModel(nn.Module):
    """Late-fusion of CNN+LSTM logits and MotionBERT ActionNet logits."""
    def __init__(self, input_dim: int, num_classes: int, mb_model: nn.Module):
        super().__init__()
        self.cnn_lstm = CNNLSTMClassifier(input_dim=input_dim, num_classes=num_classes)
        self.mb_model = mb_model
    def forward(self, x_seq, x_mb):
        logits_cnn = self.cnn_lstm(x_seq)
        logits_mb = self.mb_model(x_mb)
        # Simple average fusion
        return (logits_cnn + logits_mb) / 2.0
...
def train_multiclass(csv_path: str, epochs: int, batch_size: int, lr: float, device: str):
    ...
    # Build loaders
    def _make_loader(sequences, batch_size, shuffle=True):
        class _SeqDataset(torch.utils.data.Dataset):
            def __init__(self, seqs, label_to_idx):
                self.seqs = seqs
                self.label_to_idx = label_to_idx
        ...
        labels = sorted({lbl for _, lbl in sequences})
        label_to_idx = {lbl: i for i, lbl in enumerate(labels)}
        ds_local = _SeqDataset(sequences, label_to_idx)
        return DataLoader(ds_local, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_variable_seq), label_to_idx

    train_loader, label_to_idx = _make_loader(train_seqs, batch_size, shuffle=True)
    val_loader, _ = _make_loader(val_seqs, batch_size, shuffle=False) if val_seqs else (None, None)
    ...
    # Build MotionBERT branch
    cfg = _load_mb_config(CONFIG_YAML)
    backbone = _build_motionbert_backbone(cfg)
    mb_model = _build_actionnet(backbone, cfg, num_classes=num_classes, num_joints=NUM_JOINTS)

    # Load checkpoint strictly (no fallback)
    ...
    if isinstance(sd, dict) and 'model' in sd:
        sd = sd['model']
    try:
        mb_model.load_state_dict(sd, strict=False)
    except Exception as e:
        raise RuntimeError(f"Failed to load state_dict: {e}")

    # Fusion model
    model = FusionModel(input_dim=input_dim, num_classes=num_classes, mb_model=mb_model).to(device_t)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.enable_grad():
            for x, lengths, y in train_loader:
                x = x.to(device_t)
                y = y.to(device_t)
                x_mb = _reshape_to_actionnet_input(x, PERSONS, NUM_JOINTS, CHANNELS).to(device_t)
                optimizer.zero_grad()
                logits = model(x, x_mb)
        ...
        print(f"[CNN-LSTM+MB] Epoch {epoch}/{epochs} - loss {train_loss:.4f} acc {train_acc:.3f}{val_msg}")

        # Track best and save
        if val_loader is not None and val_acc > best_val_acc:
            best_val_acc = val_acc
            ckpt_path = OUT_DIR / "cnn_lstm_motionbert_multiclass.pt"
            torch.save({
            ...
            print(f"Saved CNN+LSTM+MotionBERT multiclass model to {ckpt_path}")

        # Early stopping
        if val_loader is not None and val_acc >= EARLY_STOP_ACC:
            print(f"[CNN-LSTM+MB] Early stop at epoch {epoch} with val acc {val_acc:.3f}")
            break

    # Learning curves
    _plot_learning_curves(train_losses, val_losses, OUT_DIR / "learning_curve.png", title="CNN+LSTM+MB Learning Curves")
    ...
    if len(train_seqs) > 0:
        tr_loader, _ = _make_loader(train_seqs, batch_size, shuffle=False)
        y_true_tr, y_pred_tr = _collect_preds(tr_loader)
        cm_tr = confusion_matrix(y_true_tr, y_pred_tr, labels=list(range(num_classes)))
        _plot_confusion(cm_tr, classes, OUT_DIR / "confusion_train.png", title="CNN+LSTM+MB Confusion (Train)")
    ...
    if val_loader is not None:
        ...
        _plot_confusion(cm_val, classes, OUT_DIR / "confusion_val.png", title="CNN+LSTM+MB Confusion (Val)")
    ...
    with torch.no_grad():
        ...
        sample_loader, _ = _make_loader(subset, batch_size=1, shuffle=False)
        for x, lengths, y in sample_loader:
            x_mb = _reshape_to_actionnet_input(x.to(device_t), PERSONS, NUM_JOINTS, CHANNELS)
            logits = model(x.to(device_t), x_mb)
